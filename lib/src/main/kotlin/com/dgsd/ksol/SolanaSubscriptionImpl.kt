...
internal class SolanaSubscriptionImpl(
  ...
  private fun createAccountSubscribeRequest(
    accountKey: PublicKey,
    commitment: Commitment
  ): RpcRequest {
  ...
  private fun onAccountNotification(notification: RpcSubscriptionNotification<AccountInfoResponse>) {
    val accountKey =
      subscriptionRegistry.getAccountKeyFromSubscriptionId(notification.params.subscriptionId)
  ...
  private fun onSignatureNotification(notification: RpcSubscriptionNotification<SignatureNotificationResponseBody>) {
    ...
    val commitment =
      subscriptionRegistry.getCommitmentFromSubscriptionId(notification.params.subscriptionId)
  ...
  private inner class SubscriptionWebSocketListener : WebSocketListener() {
    ...
    private val subscriptionConfirmationJsonAdapter =
      moshi.adapter(RpcSubscriptionConfirmation::class.java)
    private val accountNotificationJsonAdapter =
      moshi.adapter<RpcSubscriptionNotification<AccountInfoResponse>>(
